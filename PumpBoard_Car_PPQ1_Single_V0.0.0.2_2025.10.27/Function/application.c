...
=========================================================================================================*/
float                   g_f32_CurA_Ratio = 0;                                       //电流转换系数
...
#define  ANGLE_SENSOR_CUR_MODE          (0)  		//0—电压 1—电流
#define  PRESSURE_SENSOR_CUR_MODE       (0)
    
#define  ANGLE_01_SENSOR_CUR_GAIN          (1)  		//0—0.4倍 1—0.3倍
...
* Description   : 确定量程和放大倍数    小量程[0, 2A]     大量程[0, 3.2A]
* Input         : 
* Output        : 
* Return        : 
*******************************************************************************/
void CurDetectionRangeChose()
{
    u8 t_u8_Scope_A_Chose = 1;          //1-->大量程     0-->小量程
    u8 t_u8_Scope_B_Chose = 1;
	  //电磁阀A 
    if (t_u8_Scope_A_Chose == 1)       //大量程
    {
    ...
    if (t_u8_Scope_A_Chose == 0)
        g_f32_CurA_Ratio = 0.038019f;//这个还没调
    else
        g_f32_CurA_Ratio = 0.071809f;
    //放大倍数选择_B
    if (t_u8_Scope_B_Chose == 0)
        g_f32_CurB_Ratio = 0.038019f;//这个还没调
    else
...
u16 FeedbackSignalPerUnitFunc(
    ...
    u16 sensor_mid,     
    u16 sensor_max,     
    u16 output_mid      // 中间值对应的输出值
)
{	
    if(sensor_max <= sensor_mid) return 0; // 确保最大>中间值
    
    s32 raw = (s32)input_value;
    const s32 output_max = 5000;         // 固定最大输出值
    ...
    output = (output > output_max) ? output_max : (output < 0) ? 0 : output;    // 限制下限为0
...
)
{
    ...
    s32 raw = (s32)input_value;
    const s32 output_max = 5000;//10000;         // 固定最大输出值
...
{    
        ...
        float f32ShakeAmpRatio = 0;         //2%的颤振幅度
...
void TiltAng_B_FreqShakeFunc(u32 *p)
{
    #ifdef PPQ_1
        static u16 u16CycleCnt = 0;
        float f32ShakeAmpRatio = 0.1;         //2%的颤振幅度
        u16 u16AngShakeCycle = 0;          //1000ms
...
* Return        : 
*******************************************************************************/
#define LEAKAGECOMP_ENA         (0)                                                     //泄露补偿
#define PRES_D_FDB_TO_ANG_ENA   (0)                                                     //压力微分补偿
#define POWER_LIMIT             (0)                                                     //功率限制
...
void TiltAngleLoop_Func()
{
    ...
    {   
        ...
        t_s16_TiltAngRef = (s16)(InstructionSet.TiltAngRef) - g_s16_PrsCompAngVal;   //100是为了弥补稳态误差  
        ...
        if (User_Parameter.PpqEna->PwrLoop_Ena)
        {
            ...
            {
                t_f32_Pump_CC = InstructionSet.g_f32_TorqueLimt * User_Parameter.g_f32TorqueRatio / DetectorFdbVal.g_f32_PressureFdb;		//54.5867f			//计算功率限制的排量
                Limit_f32(&t_f32_Pump_CC, (float)User_Parameter.SysPara->u16PumpCC_MIN, (float)User_Parameter.SysPara->u16PumpCC_MAX);    
                s_u16_TiltAngRef_PwrLmt = (u16)((float)User_Parameter.SysPara->u16AngPerUnitScope * t_f32_Pump_CC / User_Parameter.SysPara->u16PumpCC_MAX);                          //转换到0~10000
			}
            else
            {
    ...
    if (User_Parameter.PpqEna->PumpStrt_Ena == 1)
    {
		if (User_Parameter.PpqEna->PrsLoop_Ena) 
		{
			...
			if (TimCnt.g_u16_Prs_PI_LOOP_Cnt == 0) 
			{
				...
				s_s16_PrsLoopOutput_OffsetCur = Pressure_PID_Regulator(InstructionSet.g_u16_PerUnitPrsVal+s16_PrsOffset,(u16)DetectorFdbVal.g_u32_PressureFdb, &Pressure_Loop);//设定值固定偏差
